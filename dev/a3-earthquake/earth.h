...
public:
	...
	//Returns either the spherical or rectantular positions depending on current state of the earth
	Point3 GetPosition(float lat, float longi);

	Vector3 GetNormal(float lat, float longi);

	Point3 v2p(Vector3 v) { return Point3(v[0], v[1], v[2]); }

	double isSphere() { return isSphere_; }

	void toSphere(float s);
