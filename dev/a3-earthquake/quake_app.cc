...
void QuakeApp::UpdateSimulation(double dt)  {
	...
	if (toSphere_) {
		earth_.toSphere(earth_.isSphere() + 0.05);
	}
	else if (toPlane_) {
		earth_.toSphere(earth_.isSphere() - 0.05);
	}

	if ((earth_.isSphere() == 0.0) || (earth_.isSphere() == 1.0)) {
		toSphere_ = false;
		toPlane_ = false;
	}
}
...
void QuakeApp::DrawUsingOpenGL() {
	...
	//Draw earthquakes
	double timeframeEnd = current_time_ - 31622400; //previous year
	int latestQuake = quake_db_.min_index();
	bool latestQuakeSet = false;
	Earthquake e;
	for (int i = latestQuake; i <= quake_db_.max_index(); i++) {
		latestQuakeSet = false;
		e = quake_db_.earthquake(i);
		if ((e.date().ToSeconds() >= timeframeEnd) && (e.date().ToSeconds() <= current_time_)) {
			//draw earthquake
			if (!latestQuakeSet) {
				latestQuake = i;
				latestQuakeSet = true;
			} 
			Point3 pos = earth_.GetPosition(e.latitude(), e.longitude());
			double size = (e.magnitude()-4) / (quake_db_.max_magnitude()-4) / 30.0;
			Color c = Color(0, 1, 0);
			if (e.magnitude() > 7) {
				c = Color(1, 0, 0);
			}

			Matrix4 QuakeTranslate = Matrix4::Translation(Vector3(pos[0], pos[1], pos[2]));
			Matrix4 QuakeScale = Matrix4::Scale(Vector3(size, size, size));
			quick_shapes_.DrawSphere(model_matrix * QuakeTranslate * QuakeScale, view_matrix_, proj_matrix_, c);
		}
		if (e.date().ToSeconds() > current_time_) {
			break;
		}
	}
