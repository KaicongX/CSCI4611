                           ...
                           const std::vector<Point2> &stroke2d)
{
	...
	Vector3 plane_x, plane_y, normal;
	int end = stroke2d.size()-1;
	Point3 S, E;
	ScreenPtToGround(view_matrix, proj_matrix, stroke2d[0], &S);
	ScreenPtToGround(view_matrix, proj_matrix, stroke2d[stroke2d.size()-1], &E);
	plane_x = Vector3::Normalize(S - E);
	plane_y = Vector3(0, 1, 0);
	normal = Vector3::Normalize(-look);
    
    // 2. Project the 2D stroke into 3D so that it lies on the "projection plane"
    // defined in step 1.
	std::vector<Point3> proj_points;
	for (int i = 0; i < end; i++) {
		Point3 p;
		ScreenPtToPlane(view_matrix, proj_matrix, S, normal, stroke2d[i], &p);
		proj_points.push_back(p);
	}
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
		...
		float h = hfunc(normal, proj_points, P.ClosestPointOnPlane(S, normal));
		float wd = 0;
		float d = P.DistanceToPlane(S, normal);
		if ((1 - pow(d / 5, 2)) > 0) {
			wd = 1 - pow(d / 5, 2);
		}
		/*if (h != 0) {
